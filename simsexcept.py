...
class Auto:
    car_list = {
        ...
        'Skoda': {'fuel': 60, 'durability': 150, 'cumsumption': 8},
    ...
    def speed_test(self):
        auto = Auto()
        # test yezda s dostatochnym zapasom prochnosti i topliva
        assert auto.drive() is True

        # Test poezdka bez topliva
        auto.fuel = 3
        assert auto.drive() is False

        # test yezda s ischerpannim kachestvom
        auto.durability = 0
        auto.fuel = 10
        assert auto.drive() is False
        sleep(0.747)
